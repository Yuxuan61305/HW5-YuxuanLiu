 ...
 */
export const semesterBounds = (ctx, vars, data) => data.courses.flatMap((course) => {
 ...
 */
export const prerequisiteOrder = (ctx, vars, data) => data.prereqs.map(({ course_id, prereq_id }) => vars.get(prereq_id).lt(vars.get(course_id)));
/**
 * The total credit load in any single semester must not exceed 16 credits.
 */
export const semesterCreditLimit = (ctx, vars, data) => {
    return Array.from({ length: PLANNING_SEMESTERS }, (_, i) => {
        const s = i + 1;
        const terms = data.courses.map((c) => ctx.If(vars.get(c.id).eq(ctx.Int.val(s)), ctx.Int.val(c.credits), ctx.Int.val(0)));
        const total = terms.reduce((acc, x) => acc.add(x));
 ...
 */
export const maxCoursesPerSemester = (ctx, vars, data) => {
    const MAX_COURSES = 4;
    return Array.from({ length: PLANNING_SEMESTERS }, (_, i) => {
        const s = i + 1;
        const terms = data.courses.map((c) => ctx.If(vars.get(c.id).eq(ctx.Int.val(s)), ctx.Int.val(1), ctx.Int.val(0)));
        const count = terms.reduce((acc, x) => acc.add(x));
 ...
 */
export const maxMathCoursesPerSemester = (ctx, vars, data) => {
    const MAX_MATH_COURSES = 1;
    return Array.from({ length: PLANNING_SEMESTERS }, (_, i) => {
        const s = i + 1;
        const terms = data.courses.map((c) => c.department === "MATH"

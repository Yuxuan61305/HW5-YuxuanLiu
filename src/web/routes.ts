...
import type { Course, PlanRow } from "../db/queries.js";
...
export function registerRoutes(app: Express): void {
  ...
  app.get("/courses", (_req: Request, res: Response) => {
    ...
    const prereqRows = db
      .prepare(
        `SELECT p.course_id, c.id, c.name
         FROM prerequisites p
         JOIN courses c ON c.id = p.prereq_id
         ORDER BY p.course_id`,
      )
      .all() as Array<{ course_id: string; id: string; name: string }>;
  ...
  app.get("/plan", (req: Request, res: Response) => {
    ...
    let selectedTag = req.query.tag as string | undefined;
    ...
    const prereqRows = db
      ...
      .all() as Array<{ course_id: string; prereq_id: string }>;
    ...
    const semesterMap = new Map<number, PlanRow[]>();
    ...
    const semesters = Array.from(semesterMap.entries())
      ...
      .map(([num, courses]) => ({
        number: num,
        courses: (courses as Array<Course & PlanRow>).map((c) => ({
          ...c,
          prereqs: prereqMap.get(c.course_id) ?? [],
        })),
        totalCredits: (courses as Array<Course & PlanRow>).reduce(
          (sum, c) => sum + c.credits,
          0,
        ),
      }));
  ...
  app.get("/stats", (_req: Request, res: Response) => {
    ...
    const creditsPerSemester = db
      .prepare(
        `SELECT p.semester, SUM(c.credits) AS total_credits
         FROM plan p
      ...
      .all(latestRunTag) as Array<{ semester: number; total_credits: number }>;

    const coursesPerDept = db
      .prepare(
        `SELECT c.department, COUNT(*) AS course_count, SUM(c.credits) AS total_credits
         FROM plan p
         JOIN courses c ON c.id = p.course_id
         where p.run_tag = ?
      ...
      .all(latestRunTag) as Array<{ department: string; course_count: number; total_credits: number }>;
    ...
    const busiestSemester = db
      ...
      .get(latestRunTag) as { semester: number; total_credits: number } | undefined;

    const avgResult = db
      .prepare(
        `SELECT AVG(sem_total) AS avg_credits
         FROM (
      ...
      .get(latestRunTag) as { avg_credits: number } | undefined;
